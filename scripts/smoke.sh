...
node dist/src/cli.js examples >/dev/null
node dist/src/cli.js scan examples/failing-test.log --out "$tmp_dir/triage.md" --fail-on none
node dist/src/cli.js scan examples/*.log examples/*.jsonl --json --fail-on none >"$tmp_dir/triage.json"
...
if node dist/src/cli.js scan examples/failing-test.log --fail-on fatal >/dev/null; then
